...
void writeToTxt2(const vector<vector<int> > &sorce,string filePath);//数据以空格隔开
...
{//将road中的路口信息提取出路口ID及长度
    int i,length,line,row,time,velocity;
    ...
    for(i = 0;i < roadSize;i++)
    {
        ...
        line = road[i][4];
        velocity = road[i][2];
        time = (length % velocity)?(length % velocity):((length % velocity) + 1);
        row = road[i][5];
...
//                        cout << "run5" << endl;
                ...
                runTime += (roadLength % runV) ? (roadLength / runV):(roadLength  / runV + 1 );
            }
        }
...
//    int startTime = 1;
//    int timeTemp = 0;
//    int runNum = 480;//同时跑的车辆为  runNum
//    int carID,carRuntime;
//    int carPlantime = 0;
//    int carNum = carRunTime.size();
//    int maxTime = 300;
//    int maxDt = 20;
//    if(carRunTime[0][0] == 17226)
//    {
//        runNum = 1000;
//        maxTime = 200;
//        maxDt = 30;
...
//            carPlantime = (carPlantime > carRunTime[i][4] ? carPlantime:carRunTime[i][4]);
//        }
//        startTime = startTime + carPlantime +  a* timeTemp*timeTemp + b*timeTemp;
////        cout << "startTime: " << startTime << "  carPlantime: " << carPlantime << endl;
//        for(i = k;i < n;i++)
//        {
//            carID = carRunTime[i][0];
...
typedef struct
{
    ...
    bool isRuning = 0;
    bool isEnd = 0;
    vector<vector<int> > roadOrder;//存放车辆在道路上出现的时间点
}CAR;
void CARStructClear(CAR &car)
{
    car.ID = 0;
    car.velocity = 0;
    car.planTime = 0;
    ...
    car.isRuning = 0;
    car.isEnd = 0;
    for(int i = 0;i < car.roadOrder.size();i++)
        car.roadOrder[i].clear();
    car.roadOrder.clear();
}
void creatCar(const Graph &g,const vector<vector<int> > &carRunTime,int index,CAR &car)
{
    int i,j;
    int m = 5;
    vector<int> temp;
    int carRoadNum = carRunTime[index].size();
    int roadNUm = g.road.size();
    int carNUm = g.car.size();
    int carID = carRunTime[index][0];
    int roadLength,roadV,roadID,carV,runV;
    int runTime = 0;
    car.ID = carID;
    car.endTime = carRunTime[index][3];
    for(i = 0;i < carNUm;i++)//车辆行驶速度
    {
        if(carID == g.car[i][0])
        {
            car.planTime = g.car[i][4];
            car.velocity = g.car[i][3];
            break;
        }
    }
//    for(i = 0;i < roadNUm;i++)
//    {
//        if(carRunTime[index][m] == g.road[i][0])
//        {
//            roadLength = g.road[i][1];
//            roadV = g.road[i][2];
//            break;
//        }
//    }
//    carV = car.velocity;
//    roadID = carRunTime[index][m];
//    runTime = 1;
//    temp.push_back(roadID);
//    temp.push_back(runTime);
//    car.roadOrder.push_back(temp);
//    temp.clear();
    for(m = 5;m < carRoadNum;m++)//对每条道路进行匹配，得到道路长度与最高速度
    {
//                    cout << "run4.5 "<< roadIDMatrix[j][k]<< " " << g.road[m][0]
//                         << " k m" << k << " " << m << endl;
        for(i = 0;i < roadNUm;i++)
...
void creatCarMatrix(Graph &g,const vector<vector<int> > &carRunTime,vector<CAR> &CarMatrix)
{
    int carNum = g.car.size();
    CAR car;
    int i;
    for(i = 0;i < carNum;i++)
    {
        creatCar(g,carRunTime,i,car);
        CarMatrix.push_back(car);
        CARStructClear(car);
//        cout << "carID: "<< CarMatrix[i].ID << " carvelocity: " << CarMatrix[i].velocity
//             << " carST: "<< CarMatrix[i].startTime << " carET: "<< CarMatrix[i].endTime
//             << " carIR: "<< CarMatrix[i].isRuning<< " carIE: "<< CarMatrix[i].isEnd  << endl;
//        coutVector(CarMatrix[i].roadOrder);
    }
}

int carInWhichRoad(CAR &car,int &time)//返回在时间片为time时，车辆所在路口
{
    int dt = time - car.startTime;
    int roadNum = car.roadOrder.size();
    if( (dt >= 0) && (dt <= car.endTime) )
    {
        for(int j = 0;j < roadNum;j++)
        {
            if(dt <= car.roadOrder[j][1])
                return car.roadOrder[j][0];
        }
    }
    return 0;
}
...
#define DEFAULT_OCCUPANCY 0.2
void ROADtructClear(ROAD &road)
{
    road.ID = 0;
    road.length = 1;
    road.velocity = 1;
    road.channel = 1;
    road.carNum = 0;
    road.occupancy = road.carNum / (road.length * road.channel);
}
void creatROAD(const Graph &g,vector<ROAD> &roadMatrix)
{
    int roadNum = g.road.size();
    ROAD road;
    ...
    {
        road.ID = g.road[i][0];
        road.length = g.road[i][1];
        road.velocity = g.road[i][2];
        road.channel = g.road[i][3];
        road.carNum = 0;
        road.occupancy = road.carNum / (road.length * road.channel);
        roadMatrix.push_back(road);
        ROADtructClear(road);
//        cout << "roadID: "<< roadMatrix[i].ID << " roadvelocity: " << roadMatrix[i].velocity
//             << "roadL: "<< roadMatrix[i].length << " roadchannel: " << roadMatrix[i].channel
//             << "carNum: "<< roadMatrix[i].carNum << " occupancy: " << roadMatrix[i].occupancy
//             << endl;
    }
}
void updateCarCondition(vector<CAR> &carMatrix,vector<ROAD> &roadMatrix,int index,int time,int &carCount)//更新车辆状态
{
    int i;
    int carNum = carMatrix.size();
    int roadID = roadMatrix[index].ID;
    int roadidTemp;
//    cout << "time: " << time << "  roadID: " << roadID << " occupancy: " <<roadMatrix[index].occupancy << endl;
    roadMatrix[index].carNum = 0;
    roadMatrix[index].occupancy = 0.0;
//    if(roadMatrix[index].occupancy > DEFAULT_OCCUPANCY)
//            return;
    for(i = 0;i < carNum;i++)//更新已经在行驶的车辆状态
    {
        if(carMatrix[i].isRuning && !carMatrix[i].isEnd)//已经在路上但未到达终点的车辆
        {
            roadidTemp = carInWhichRoad(carMatrix[i],time);
            if(roadidTemp == roadID)//行驶中的车辆在此时间片所在路径是roadMatrix[index]
            {
                ++(roadMatrix[index].carNum);
                roadMatrix[index].occupancy = (roadMatrix[index].carNum*0.001) /
                                              (0.001 * roadMatrix[index].length);// * roadMatrix[index].channel);
//                cout << " 1 " << carMatrix[i].ID;
            }
            if((time - carMatrix[i].startTime) >= carMatrix[i].endTime)
            {
                carMatrix[i].isEnd = 1;
                carMatrix[i].isRuning = 0;
            }
    ...
    for(i = 0;i < carNum;i++)
    {
        if( !carMatrix[i].isRuning && !carMatrix[i].isEnd)//未出发的车辆
        {
            roadidTemp = carMatrix[i].roadOrder[0][0];
            if(roadidTemp == roadID)
            {
                carMatrix[i].isRuning = 1;
                carMatrix[i].startTime = time + carMatrix[i].planTime ;//+ 0.2*carMatrix[i].endTime ;//(time > carMatrix[i].planTime)?time:carMatrix[i].planTime;
                ++roadMatrix[index].carNum;
                roadMatrix[index].occupancy = (roadMatrix[index].carNum*0.001) /
                                              (0.001 * roadMatrix[index].length);// * roadMatrix[index].channel*0.001);
                ++carCount;
//                cout << " 2 " << carMatrix[i].ID;
            }
        }
        if(roadMatrix[index].occupancy > DEFAULT_OCCUPANCY)
            break;
    }
//    cout << "time: " << time << "  roadID: " << roadID << " occupancy: "
//         <<roadMatrix[index].occupancy <<" " << roadMatrix[index].carNum<< endl;
//    cout << endl;
}

//version4 道路占有率
void creatRunOrder(vector<CAR> &CarMatrix,vector<ROAD> &roadMatrix,vector<vector<int> > &CarRunOrder)
{
    int i,j,n;
    int beginTime = 0;
    int time = 0;
    int roadNum = roadMatrix.size();
    int carNum = CarMatrix.size();
    int carCount = 0;
    vector<int> temp;
//    cout << "carNum: "<< carNum << endl;
    while(carCount != carNum)
    {
        time++;
        for(i = 0; i < roadNum;++i)
        {
            updateCarCondition(CarMatrix,roadMatrix,i,time,carCount);
        }
    }
    for(i = 0;i < carNum;i++)
    {
        temp.push_back(CarMatrix[i].ID);
        temp.push_back(CarMatrix[i].startTime);
        n = CarMatrix[i].roadOrder.size();
        for(j = 0;j < n;j++ )
        {
            temp.push_back(CarMatrix[i].roadOrder[j][0]);
//            cout << "i : " << i << " ";
//            coutVector(temp);
        }
        CarRunOrder.push_back(temp);
...
//    writeToTxt2(carRuntime,"../config/carRuntime.txt");
    sort(carRuntime.begin(),carRuntime.end(),comp3);
////////    sort(carRuntime.begin(),carRuntime.end(),comp4);//根据车辆在最短路径上行驶时间进行排序
    creatCarRunOrder(carRuntime,CarRunOrder);       //车辆出发时间顺序
    writeToTxt(CarRunOrder,answerPath);             //写入answer.txt

//    writeToTxt2(carRuntime,"../config/carRuntimesort.txt");
//    writeToTxt2(pathTable,"../config/RoadIDMatrix.txt");
//    writeToTxt2(path,"../config/RoadID.txt");

//    CAR car;

//    creatCar(g,carRuntime,0,car);
//    for(i = 0;i < 8;i++)
//    {
//        int roadid;
//        roadid = carInWhichRoad(car,i);
//        cout << "time: " << i << " roadid: " << roadid << endl;
//    }
//    cout << car.ID << " " << car.velocity << endl;
//    coutVector(car.roadOrder);
//    coutVector(carRuntime);

//
//    vector<CAR> CarMatrix;
//    creatCarMatrix(g,carRuntime,CarMatrix);
//    vector<ROAD> road;
//    creatROAD(g,road);
//    creatRunOrder(CarMatrix,road,CarRunOrder);
//    writeToTxt(CarRunOrder,answerPath);
//    sort(CarRunOrder.begin(),CarRunOrder.end(),comp1);
//    writeToTxt(CarRunOrder,"../config/ans.txt");
//    int index = 0;
//    int time = 0;
//    int carcount = 0;
//    while(carcount < 4000)
...
//        {
//            updateCarCondition(CarMatrix,road,i,time,carcount);
//        }
